...
export async function addComment(req, res) {
    try{
        const {text, userId, videoId, username}= req.body;
        const newComment= new commentModel({
            text, userId, videoId, username
        });
        await newComment.save();
        await videoModel.findByIdAndUpdate(videoId, {$push: {comments: newComment._id}}, {new: true});
...
export async function editComment(req, res){
    try{
        const {text}= req.body;
        const updatedComment= await commentModel.findByIdAndUpdate(req.params.id, {text}, {new: true});
        if(!updatedComment){
            return res.status(404).json({error: "Comment not found"});
        }
        res.status(200).json({message: "comment Updated", text: updatedComment.text});
    }catch(error){
