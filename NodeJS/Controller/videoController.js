...
export async function likeVideo(req, res) {
    try{
        const {userId}= req.body; //Assume userId comes from frontend
...
export async function dislikeVideo(req, res) {
    try {
      const {userId}= req.body; //Asume userId comes from forntend  
      const video = await videoModel.findById(req.params.id);
      if (!video) return res.status(404).json({ message: "Video not found" });
...
export async function uploadVideo(req, res) {
    try{
        const {title, thumbnailUrl, videoUrl, description, category, channelId, uploader, views, likes, dislikes, comments}= req.body;
        const newVideo= new videoModel({
            title, thumbnailUrl, videoUrl, description, category, channelId, uploader, uploadDate: new Date(), views, likes, dislikes, comments
        });
        await newVideo.save();
...
export async function updateVideo(req, res){
    try{
        const updatedVideo= await videoModel.findByIdAndUpdate(req.params.id, req.body);
        if(!updatedVideo){
            return res.status(404).json({error: "Video not found"});
        }
